...
def configure(distro: str, **kwargs) -> bool:  # noqa: ARG001, ANN003
    ...
    """
    logger.info("Optimizing package manager configuration for %s...", distro)

    if distro in ("fedora", "rhel", "centos", "nobara"):
        success = _optimize_dnf()
    elif distro in ("arch", "archlinux", "manjaro", "cachyos"):
        success = _optimize_pacman()
    else:
...
def _optimize_dnf() -> bool:
    ...
    if not _create_backup(dnf_conf):
    ...
    for key, value in settings.items():
        if not _add_or_update_setting(dnf_conf, key, value):
            return False
...
def _optimize_pacman() -> bool:
    ...
    if not _create_backup(pacman_conf):
    ...
    for key, value in settings.items():
        if not _add_or_update_setting(
            pacman_conf, key, value, separator=" = "
        ):
            return False

    if not _enable_pacman_color(pacman_conf):
        return False
...
def _create_backup(config_file: Path) -> bool:
...
def _add_or_update_setting(
    config_file: Path, key: str, value: str, separator: str = "="
) -> bool:
...
def _enable_pacman_color(pacman_conf: Path) -> bool:
    ...
    try:
        ...
        logger.exception("Error enabling pacman color")
        return False

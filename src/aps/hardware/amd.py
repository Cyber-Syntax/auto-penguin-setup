...
class AMDConfig(BaseHardwareConfig):
    ...
    def _is_amd_cpu(self) -> bool:
        ...
        try:
            ...
            self.logger.warning("Cannot detect CPU type - /proc/cpuinfo not found")
            return False
    ...
    def setup_zenpower(self) -> bool:
        """Setup zenpower for AMD Ryzen 5000 series.
        ...
        self.logger.debug("Creating k10temp blacklist file at %s...", blacklist_file)
    ...
    def _setup_zenpower_fedora(self) -> bool:
        ...
        if result.returncode != 0:
            self.logger.error("Failed to install zenpower3 and zenmonitor3 packages")
            return False

        return self._load_zenpower_module()

    def _setup_zenpower_arch(self) -> bool:
        """Setup zenpower on Arch Linux.
        ...
        for helper in ["paru", "yay"]:
            result = subprocess.run(["which", helper], capture_output=True, check=False)
            if result.returncode == 0:
                aur_helper = helper
                break
        ...
        if result.returncode != 0:
            self.logger.warning("Failed to install zenmonitor3 from AUR (optional)")

        return self._load_zenpower_module()

    def _setup_zenpower_debian(self) -> bool:
        """Setup zenpower on Debian/Ubuntu.

        Returns:
            True if successful
        """
        self.logger.warning("Zenpower is not officially packaged for Debian/Ubuntu")
    ...
    def _load_zenpower_module(self) -> bool:
        ...
        if result.returncode != 0:
            ...
            self.logger.info(
                "A system restart may be required to load the zenpower3 module, "

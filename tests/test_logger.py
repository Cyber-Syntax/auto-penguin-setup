...
class TestLoggingIntegration:
    ...
    def test_no_duplicate_messages_normal_mode(self, mock_home: Path) -> None:  # noqa: ARG002
        """Test that messages don't appear multiple times in normal mode."""
        stderr_capture = StringIO()
        with patch.object(sys, "stderr", stderr_capture):
            setup_logging(verbose=False)
    ...
    def test_no_duplicate_messages_verbose_mode(self, mock_home: Path) -> None:  # noqa: ARG002
        """Test that messages don't appear multiple times in verbose mode."""
        stderr_capture = StringIO()
        with patch.object(sys, "stderr", stderr_capture):
            setup_logging(verbose=True)
            logger = get_logger("test")
    ...
    def test_multiple_loggers_use_same_configuration(self, mock_home: Path) -> None:  # noqa: ARG002
